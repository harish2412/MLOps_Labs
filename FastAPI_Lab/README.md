Changes in this lab assignment:
    ...
    1. Switched from Iris to the Breast Cancer dataset.
    2. Used a Random Forest model instead of a Decision Tree.
    3. Training now uses all 30 features from the dataset.
    4. The model is saved as model/breast_rf.pkl.
    5. The train script uses a stratified train test split with a fixed seed for reproducibility.
    6. The API request body now accepts a single list named features with exactly 30 numbers.
    7. Predictions use predict_proba with a 0.5 threshold (0 = malignant, 1 = benign).
    8. Screenshots in assets show the updated docs and a malignant example
...
1. Training a Random Forest classifier on the Breast Cancer dataset.
...
1. First step is to train a Random Forest classifier (you might already have model/breast_rf.pkl, but let’s train a fresh one). To do this, move into **src/** folder with
    ```bash
    cd src
    ```
...
FastAPI will:
 ...
 ~ Validate that exactly 30 numbers are provided (else it returns 422)
...
When you set response_model=CancerResponse, FastAPI serializes the output and documents it in the OpenAPI/Swagger UI.
...
2. **Data Conversion**: Once the request body is read, FastAPI utilizes Pydantic models to parse the JSON data. Pydantic attempts to construct an instance of the specified model using the data from the request body. During this instantiation, Pydantic converts the JSON data into the proper Python data types as declared in the model.
    - For instance, if the JSON has "features": ["0.1", "0.2", ...] (strings) and the model expects floats, Pydantic converts them to floats. If conversion isn’t possible, it raises a validation error.
...
async def read_item(item_id: int):
    item = get_item_by_id(item_id)  # your lookup
    if item is None:
